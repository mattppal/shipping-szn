...
Guidelines from `.cursor/rules/` - these apply to all development:

## Python Code Style
- **Formatting:** Black (88 char line length)
- **Type hints:** Include for function parameters and returns
- **Strings:** Double quotes, prefer f-strings
- **Imports:** Group as standard library → third-party → local
- **Environment:** Load `dotenv` at module top before `os.getenv()`
- **Naming:** Functions `snake_case`, Classes `PascalCase`, Constants `UPPER_SNAKE_CASE`

## Claude Agent SDK Patterns
- Use `@tool` decorator with `is_error` boolean in return dict
- `AgentDefinition` with clear `description` and comprehensive `prompt`
- Model selection: `sonnet` for complex tasks, `haiku` for simpler tasks
- Define MCP servers in `servers/config.py`
- Permission groups follow principle of least privilege

## Documentation Standards
- **Images:** Use `/images/changelog/YYYY-MM-DD/filename` (not `./media/...`)
- **Person:** Second person ("you"), never "we"
- **Products:** Capitalize Replit products (Agent, Assistant, Workspace)
- **Terminology:** "publishing" not "deployment"
...
- Never expose tokens in logs/errors
